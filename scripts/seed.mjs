...
if (EMIT_SQL) {
  // Emit a single SQL file that can be pasted into the Supabase SQL editor.
  // Avoids needing Node, a service role key, or network access to seed.
  const q = (v) =>
    v === null || v === undefined ? 'null' : `'${String(v).replace(/'/g, "''")}'`;
  const id = () => crypto.randomUUID();
  const L = [];
  L.push('-- Generated by: node scripts/seed.mjs --emit-sql');
  L.push('-- Loads seed.json. Run after the migrations, once, on an empty database.');
  L.push('begin;');
  L.push('');

  const pid = new Map(people.map((p) => [p.name, id()]));
  L.push('insert into people (id, name, email, role, active) values');
  L.push(people.map((p) => `  (${q(pid.get(p.name))}, ${q(p.name)}, ${q(p.email)}, ${q(p.role)}, ${p.active})`).join(',\n') + ';');
  L.push('');

  const gid = new Map(stageGroups.map((g) => [g.name, id()]));
  L.push('insert into stage_groups (id, name, seq, active) values');
  L.push(stageGroups.map((g) => `  (${q(gid.get(g.name))}, ${q(g.name)}, ${g.seq}, ${g.active})`).join(',\n') + ';');
  L.push('');

  const sid = new Map(substages.map((s2) => [`${s2.group} :: ${s2.name}`, id()]));
  L.push('insert into substages (id, stage_group_id, name, seq, planned_weeks, active) values');
  L.push(substages.map((s2) =>
    `  (${q(sid.get(`${s2.group} :: ${s2.name}`))}, ${q(gid.get(s2.group))}, ${q(s2.name)}, ${s2.seq}, ${s2.planned_weeks ?? 'null'}, ${s2.active})`
  ).join(',\n') + ';');
  L.push('');

  const dels = substages.flatMap((s2) => s2.deliverables.map((d) => ({ sub: `${s2.group} :: ${s2.name}`, ...d })));
  L.push('insert into deliverables (id, substage_id, name, seq, active) values');
  L.push(dels.map((d) => `  (${q(id())}, ${q(sid.get(d.sub))}, ${q(d.name)}, ${d.seq}, ${d.active})`).join(',\n') + ';');
  L.push('');

  const prid = new Map(projects.map((p) => [p.key, id()]));
  L.push('insert into projects (id, name, type, status, priority, code, client_name, site_location) values');
  L.push(projects.map((p) =>
    `  (${q(prid.get(p.key))}, ${q(p.name)}, ${q(p.type)}, ${q(p.status)}, ${p.priority ?? 'null'}, ${q(p.code)}, ${q(p.client_name)}, ${q(p.site_location)})`
  ).join(',\n') + ';');
  L.push('');

  L.push('insert into assignments (id, project_id, person_id, role_code) values');
  L.push(assignments.map((a) =>
    `  (${q(id())}, ${q(prid.get(a.project))}, ${q(pid.get(a.person))}, ${q(a.role_code)})`
  ).join(',\n') + ';');
  L.push('');

  L.push('insert into project_substage (id, project_id, substage_id, status, started_on, concluded_on, target_date) values');
  L.push(projectSubstages.map((r) =>
    `  (${q(id())}, ${q(prid.get(r.project))}, ${q(sid.get(r.substage))}, ${q(r.status)}, ${q(r.started_on)}, ${q(r.concluded_on)}, ${q(r.target_date)})`
  ).join(',\n') + ';');
  L.push('');

  // Give every seeded project the full section structure, using exactly the
  // rule migration 0011 uses. The migration itself is a no-op on a fresh
  // database because it runs before this file; calling the function here is
  // what makes a new deployment and a backfilled one identical.
  L.push('-- Full section structure for every project. See migration 0011.');
  L.push("select ensure_project_structure(id) from projects where status not in ('cancelled', 'completed');");
  L.push('');
  L.push('commit;');
  ...
  console.log(`\nwrote supabase/seed.sql  (${L.join('\n').length.toLocaleString()} bytes)`);
  console.log('Run it in the Supabase SQL editor after migrations 0001-0004.\n');
